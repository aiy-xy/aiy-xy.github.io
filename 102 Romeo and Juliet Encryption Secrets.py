...
def ROMEO():
    ...
    encrypted_nums = [(num * key[i]) % 256 for i, num in enumerate(nums)]
    ...
    decrypted_nums = [(num * secrets.token_bytes(16)[i]) % 256 for i, num in enumerate(received_nums)]
...
def JULIET():
    ...
    encrypted_nums = [(num * key[i]) % 256 for i, num in enumerate(received_nums)]
